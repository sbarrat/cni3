...
//***********************************************************************************************/
/* @autor: Ruben Lacasa <ruben@ensenalia.com>
 * @file:generator.php
 * @description:Corazon de la aplicacion, creado en Marzo de 2007. 
 * 				Original 964 Lineas, Modificacion en 2011
 */
...
switch ($_POST ["opcion"]) {
	case 0 :
		$respuesta = generador ( $_POST );
		break;
	case 1 :
		$respuesta = buscador ( $_POST );
		break;
	case 2 :
		$respuesta = formulario ( $_POST );
		break;
	case 3 :
		$respuesta = subformulario ( $_POST );
		break;
	case 4 :
		$respuesta = actualiza ( $_POST );
		break;
	case 5 :
		$respuesta = nuevo ( $_POST );
		break;
	case 6 :
		$respuesta = agrega_registro ( $_POST );
		break;
	case 7 :
		$respuesta = borra_registro ( $_POST );
		break;
	case 8 :
		$respuesta = frm_srv_fijos ( $_POST );
		break;
	case 9 :
		$respuesta = cambia_los_otros ( $_POST );
		break;
	case 10 :
		$respuesta = agrega_srv_fijo ( $_POST );
		break;
	case 11 :
		$respuesta = borra_srv_fijo ( $_POST );
		break;
	case 12 :
		$respuesta = actualiza_srv_fijo ( $_POST );
		break;
}
echo $respuesta;

/**
 * Opcion 0. Funcion que genera el menu de la aplicacion 
 * @param array $vars
 * @return string
 * */
function generador($vars)
{
	...
	if($vars["codigo"] == 6)
	{
		...
		<div class="gestion_app">
			...
			<span class="boton" onclick="hacerBackup()">
				<acronym title="Hacer Copia">Hacer copia</acronym>
			</span>
			<span class="boton" onclick="listaBackup()">
				<acronym title="Listado Copias">Listado de Copias</acronym>
			</span>
			<span class="boton" onclick="revisarTablas()">
				<acronym title="Revisar Tablas">Revisar Tablas</acronym>
			</span>
			<span class="boton" onclick="repararTablas()">
				<acronym title="Reparar Tablas">Reparar Tablas</acronym>
			</span>
			<span class="boton" onclick="optimizar_tablas()">
				<acronym title="Hacer Copia">Optimizar Tablas</acronym>
			</span>
		</div>
		<div class="gestionApp">
			Datos Categorias:
			<span class="boton" onclick="categorias(1)">
				Categorias Servicios
			</span>
			<span class="boton" onclick="categorias(2)">
				Categorias Clientes
			</span>
		</div>
		<div class="gestionApp">
			Telefonos Centro: 
			<span class="boton" onclick="formularioTelefonos()">
				Gestion Telefonos Centro
			</span>
		</div>
		<div class="gestionApp">
			Listado Despachos y Domiciliados:
			<span class="boton" onclick="consultaEspecial()">
		...
		<div id="listadoCopias"></div>
		<div id="estadoCopia"></div>
		<div id="estadoTablas"></div>';
	}
	else
	{
		$aplicacion->pagina ( $vars ['codigo'] );
	    $resultado = $aplicacion->datos();
	    $valorSeccion = $resultado[0]['pagina'];
	    $seccion = ucfirst($valorSeccion);
		$tabla .= "<div id='botoneria'>
		<span class='titulo_categoria'>Seleccione {$seccion}:</span>
		<input type='hidden' id='tabla' value='{$valorSeccion}' />
		<input type='hidden' id='nuevo' value='{$vars["codigo"]}' />
		<input type='text' id='texto' autocomplete='off' onkeyup='busca()'/>
		<input class='boton' type='submit' onclick='busca()' 
		value='[M]Mostrar Busqueda'>
		...
		if($vars["codigo"] == 1)
		{
			...
			<input class='boton' type=submit 
			onclick=popUp('almacen/index.php') 
			value = 'Almacenaje' />
			<input class='boton' type=submit 
			onclick=popUp('agenda/index.php') 
			value = 'Agenda' />
			<input class='boton' type=submit 
			onclick=popUp('entradas2/index.php') 
			value = 'Entradas' />";
		}
		$tabla .= "</div>";
...
/**
 * Opcion 1. Funcion que busca en la base de datos un valor
 * @param array $v../application/inc/Aplicacion.php
 */
function buscador($vars)
{
	include_once '../../inc/clases/Aplicacion.php';
	$aplicacion = new Aplicacion();
	$aplicacion->buscador($vars);
	$muestra = "";
	
	if($vars["texto"] != "")
	{
	    $muestra .= 
	    "<input class='boton' type='button' onclick='cierra_frm_busca()' 
	    value='[X]Cerrar'>";
	    foreach($aplicacion->datos() as $resultado){
	        $dato = utf8_encode(
...
function colorCabezera($tabla,$vars)
{
	$color = "#7d0063";
    if($tabla == 'clientes'){
        if(preg_match("/despacho/i", $vars["Categoria"]) >= 1)
            $color="#69C";
        if(preg_match("/domicili/i", $vars["Categoria"]) >= 1)
            $color="#F90";    
    }
    return $color;
}
...
/**
 * Funcion que devuelve el codigo de negocio
 * @param string $idemp
 * @return string
 */
...
/*
 * Function tipo_campo(nombre_campo,nombre_tabla,valor_campo,opcion,orden_en_la_lista)
 */
function tipo_campo($campo,$tabla,$valor,$opcion,$orden)
{
	...
	switch($resultado["tipo"])
	{
					...
					$valor = traduce($valor);
		 			if (($tabla =='z_sercont') && ($resultado["campoo"]=='valor'))
						$cadena ="<div id='tipo_teleco'>
						 ...
						 id='{$resultado["variable"]}' name='{$resultado["campoo"]}' 
		...
		break;
		case "textarea":$valor = traduce($valor);
						$cadena = "<textarea id='{$resultado["variable"]}' 
						name='{$resultado["campoo"]}' rows='{$resultado["size"]}' 
						cols='46' tabindex='{$i}'>{$valor}</textarea>";
		break;
		case "checkbox":$chequeado = "";
						if ($valor == 0)
							$chequeado = 'checked';
		...
		break;
		case "date":    $valor = cambiaf($valor);
						$cadena = "<input type='text' id='{$resultado["variable"]}' 
						 name='{$resultado["campoo"]}' size = '{$resultado["size"]}'
						 value='{$valor}' tabindex='{$i}'/>";
						$cadena .= "&nbsp;&nbsp;<button TYPE='button' 
						 class='calendario' id='f_trigger_{$resultado["variable"]}' 
						 tabindex='{$i}'></button>";
		break;
		case "select":  $valor = traduce($valor);
						...
						if ($tabla =='z_sercont') //caso del z_sercont
							...
							$cadena ="<select id='{$resultado["variable"]}' 
							name='{$resultado["campoo"]}' tabindex='{$i}'>";
						$titulo = traduce($resultado["campoo"]);	
						$cadena .="<option value='0'>-::{$titulo}::-</option>";
						while (($resultado = mysql_fetch_array($consulta)) == TRUE)
						{
							$marcado = "";
							$seleccionable = traduce($resultado[1]);
							if ($seleccionable == $valor)
								$marcado = 'selected';
							$cadena .= "<option ".$marcado." value='{$seleccionable}'>
	...
	switch($resultado["enlace"])//generamos el enlace de conexion o bien a web o envio de correo
		{
			case "web":$cadena .="<a href='http://{$valor}' target='_blank'>
...
function submenus($tabla)
{
	...
	$sql = "SELECT `id` FROM `menus` " .
	 "WHERE `pagina` LIKE '{$tabla["tabla"]}'";
	$consulta = mysql_db_query($dbname,$sql,$con);
	$resultado = mysql_fetch_array($consulta);
	$sql = "SELECT * FROM `submenus` " .
	 "WHERE `menu` LIKE '{$resultado[0]}'";
	$consulta = mysql_db_query($dbname,$sql,$con);
...
function actualiza($vars)
{
	...
	//../application/inc/variables.phpen entrar en el bucle
...
//formulario de registro nuevo, aq../application/inc/variables.phpgregar	
function nuevo($vars)
{
	include_once '../../inc/variables.php';
...
function agrega_registro($vars)
{
	...
	if(($consulta = mysql_db_query($dbname,$sql,$con)) == TRUE)
		return "<img src='".OK."' alt='Registro ".$tipo."' width='24'/> Registro ".$tipo."&nbsp;&nbsp;<p/>";
	else
		return "<img src='".NOK."' alt='ERROR' width='24'/> ERROR&nbsp;&nbsp;<p/>";
}
//***************************************************************************../application/inc/variables.php*******/
...
<table cellpadding='0px' cellspacing='1px' class='formulario' ><tr>";
	...
	switch($resultado[0])
	{
		case("facturacion"):{ $cadena .= subform($listado,$resultado[0],$registro,$marcado);
		...
		case("z_facturacion"):$cadena .= subform($listado,$resultado[0],$registro,$marcado);break;
		case("cfm"):$cadena.= subform($listado,$resultado[0],$registro,$marcado);break;
		case("tllamadas"):$cadena.= subform($listado,$resultado[0],$registro,$marcado);break;
		//case("pproveedores"):subform($form,$resultado[0],$registro,$vars[marcado]) ."".sublist($listado,$resultado[0]);break;
		default: $cadena .= subform($formulario,$resultado[0],$registro,$marcado) ."".sublist($listado,$resultado[0]);break;
	}
	$cadena .= "</table></form>";
	return $cadena;
}
//***********************************************************************************************/
function subform($sql,$tabla,$registro,$marcado = FALSE)
{
	//$cadena = $sql;
	//if(isset($marcado))
	//$cadena .= "marcado";
	...
	switch($tabla)
	{
		...
		default:{$cadena_opcion = "";if($marcado)$tipo = "Actualizar"; else $tipo = "nuevo";}break;
	}
